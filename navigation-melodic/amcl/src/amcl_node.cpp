...
class AmclNode
{
  ...
  private:
    ...
    void AutoReInitialPose(geometry_msgs::PoseWithCovarianceStamped & last_pose);   //add by whz 2020.09.16
...
main(int argc, char** argv)
{
  ...
  //信号函数，信号SIGINT指中断信号，一般执行"ctrl+c"时触发
...
AmclNode::laserReceived(const sensor_msgs::LaserScanConstPtr& laser_scan)
{
  ...
  if(pf_init_)
  {
    ...
    //add by whz 2020.09.16
    pf_sample_set_t *tmp = pf_->sets + pf_->current_set;
    // if (tmp->sample_count < (min_particles_ + 200) && m_force_update == true)   //add at 2022.09.16
    // {
